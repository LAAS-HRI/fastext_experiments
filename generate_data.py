...
def load_templates(templates_file_path):
    ...
    with open(templates_file_path, "r") as csvfile:
        ...
        for row in reader:
            ...
            templates_by_index[index]["Templates"] = row["Templates"]
            templates_by_index[index]["Labels"] = row["Labels"]
            index += 1
    return templates_by_index
...
def generate_data_pairs(variables, templates, individuals, nb_examples_per_template=600):
    ...
    for i in range(0, len(templates)):
        ...
        for j in range(0, nb_examples_per_template):
            ...
            labels, target = generate_random_pair(variables_list, source, target, individuals)
...
def generate_random_pair(variables, source, target, individuals):
    if len(variables) > 0:
        for i in range(0, len(variables)):
            ...
            individual = individuals[variables[i]][index]
            target = target.replace(".", ",")
            target = target.replace("<"+str(chr(65+i))+">", individual)
            source = source.replace("<"+str(chr(65+i))+">", individual)
